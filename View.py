...
class SnakeViewer:
	...
	def AddMaze(self, maze):
		...
		for i in range(1,maze.HEIGHT*Units -WI, WI-1):
			self.canvas.create_rectangle(0,i,WI+2,i+WI,fill = borderColor, tags = 'wall')
			self.canvas.create_rectangle((maze.WIDTH)*Units - WI+2,i,maze.WIDTH*Units+2,i+WI,fill = borderColor, tags = 'wall')

		for i in range(1,maze.WIDTH*Units, WI-1):
			self.canvas.create_rectangle(i, 1, i+WI, WI, fill=borderColor, tags='wall')
